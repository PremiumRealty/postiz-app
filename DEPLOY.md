...
**Сборка требует ~4 ГБ ОЗУ** — фронтенд собирается с
`--max-old-space-size=4096`. На VM из раздела 6 (2 ГБ) сборка упадёт по OOM.
Варианты: временно добавить swap, либо собрать на другой машине и перенести
образ:
