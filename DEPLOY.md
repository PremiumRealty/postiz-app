...
**Сборке нужно ~4 ГБ ОЗУ и ~25 ГБ свободного диска.** Память — потому что
фронтенд собирается с `--max-old-space-size=4096`; на VM из раздела 6 (2 ГБ)
сборка упадёт по OOM.

Диск — аппетит неочевидный: на VM с диском 41 ГБ сборка оставила после себя
10.8 ГБ кеша buildx и всё равно упала с `no space left on device`, стартовав
примерно с 30 ГБ свободных. Причина в том, что под конец одновременно на диске
лежат кеш сборки, экспортируемый образ и его распаковка. Место кончается на
последнем шаге, когда всё уже собрано, поэтому `make image` проверяет свободное
место заранее и отказывается стартовать, если его мало.

Перед повторной сборкой имеет смысл чистить кеш — он растёт от сборки к сборке
и обычно и есть главный пожиратель:

```bash
docker system df              # что занимает место
docker builder prune -af      # кеш сборок; на практике освобождает больше всего
docker image prune -f         # висячие образы (без -a, чтобы не задеть рабочие)
du -sh backups/*              # старые бэкапы тоже накапливаются
```

Порог меняется через `make image IMAGE_MIN_FREE_GB=15`, а совсем отключается
